...
class LLaMAModel:
    ...
    def _generate_new_object(self, current_object: str, image_description: str) -> str:
        """
        ...
        """
        prompt_1 = (
            "USER: Imagine you are a object replacer. Your task is generating a replacement object instead of the existing object on the scene."
        ...
        new_object = self._infer(prompt_1)
    ...
    def generate_prompt(
        ...
        if new_object is None:
            new_object = self._generate_new_object(current_object, image_description)
            new_object = new_object[2:] if new_object.startswith("a ") else new_object

        prompt = (
            f"USER: Imagine that you want to describe the {new_object}'s appearance to an artist in one sentence, under 15 words."
